  ...
  var codeVal = num.toString();
   ...
   $('#inputId').on({
      keyup:function(event){
          ...
          var regExp=/^((?=.*[A-Za-z])+(?=.*[0-9])*)[^\s][A-Za-z0-9]{5,}$/g;
   ...
   $('.phone-btn').on({
    click:function(e){
      ...
      if(regExp.test(PhoneValue)===false){
        ...
        $('.receive-info').show();
        $(this).addClass('on');
        modal('잘못된 휴대폰 번호 입니다. 확인 후 다시 시도해 주세요.');
        phoneOk;
      }
      else{
    ...
    $('.cert-confirm-btn').on({
      click:function(e){
        ...
        if(CodeValue===codeVal){
          ...
          $('.guide-receive span').show().text('인증번호를 확인해 주세요.');
